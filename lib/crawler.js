...
module.exports = (function() {
    ...
    var executeSearch = function (search) {
        ...
        var browseUrls = function () {
            ...
            search.urls.forEach(function (uri, i, urls) {
                chain = chain
                    .then(function() {
                        return browsePages(uri);
                    })
                    .then(persistAds);
        ...
        var browsePages = function (uri) { 
            var defer = Q.defer();
            var adsToBeSaved = [];

            var loadPage = function (uri) {
     ...
     * @return {Q.defer.promise} Return a promise
     */
    var getContent = function (uri, jquery) {
        var defer = Q.defer();
        var counter = 0;
        var max = 5;
        var uriDatas = url.parse(uri);

        jquery = jquery || false;

        logger.info('Loading content from', uri);

        var load = function () {

            proxy = proxies.getNext();
            logger.info('  -> Use proxy "'+proxy.ip+':'+proxy.port+'"');
            counter++;
            ...
            var req = http.get(
                ...
                function(res) {
                    ...
                    res.on('end', function () {
                        ...
                        defer.resolve(jquery ? $(body) : body);
                    });
                }
            );
    ...
    var extractNextPage = function ($content) {
        ...
        if(($paging = $content.find('#paging')).length) {
            var uri = $paging.find('li.selected').next().find('a:first').attr('href');
        }
    ...
    var extractAdFromListing = function ($content) {
        ...
        // Find uri
        var uri = $content.parent().attr('href');

        // Find uid
        var matches = uri.match(/([0-9]+)\.htm/);
